...
依 Socket.IO 行為與 client 設定而定。若 server 主動斷線是因驗證失敗，應先更新 token 或重新登入。另外，舊版 client 也可能在特定錯誤後停止重連，例如 \`python-socketio\` 4.5.1 收到 HTTP 503 後就不再重試，排查與手動重連寫法可以參考 [Python Socket.IO Client 4.5.1 不會自動重連](/post/python-socketio-client-451-reconnect-issue)。
