...
在 TensorFlow 中，有一種特殊的張量類型叫做梯度張量，可以用來計算模型的梯度。這篇文章說明 TensorFlow 自動求導（自動微分）機制的原理、\`GradientTape\` 的標準使用流程，並用一個最小範例示範如何求出函數在特定點的導數——這正是訓練神經網路時反向傳播的基礎，算出來的梯度會交給[梯度下降法](/post/gradient-descent-introduction)更新參數。
